...
class Captcha {
    ...
    public function getVerifyCode($regenerate = false) {
        ...
        if ($value === null || $regenerate) {
            $session->add(self::KEY, $this->generateVerifyCode());
            $session->add(self::KEY . 'count', 1);
        }
    ...
    public function validate($input, $caseSensitive) {
        ...
        $session = Context::getInstance()->getSession();
        $name = self::KEY . 'count';
        $session->add($name, $limit = $session->get($name) + 1);
        if ($limit > $this->testLimit && $this->testLimit > 0)
            $this->getVerifyCode(true);
        return $valid;
    }

    /**
     * Generates a newest verification code.
    ...
    protected function renderImage($code) {
        if (class_exists('ImagickPixel')) {
            ...
            throw new Exception('Image GD or Imagick library not found!');
    }
